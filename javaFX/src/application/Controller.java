...
public class Controller implements Initializable
{
	...
	public void initialize(URL location, ResourceBundle resources)
	{
		 ...
		 */
		specimen.bind(expressionsModel.specimenProperty());
		 ...
		 * 	- quitButton
		 */
		styleableButtons.addAll(List.of(
			addButton,
			openButton,
			saveButton,
			selectAllButton,
		 ...
		 * 	TerminalType.ALL
		 */
		terminalTypeCombobox.setItems(FXCollections.observableArrayList(TerminalType.all()));
		 ...
		 */
		binaryTypeCombobox.setItems(FXCollections.observableArrayList(BinaryOperatorRules.all()));
		 ...
		 */
		expressionsTableView.getSelectionModel().setSelectionMode(javafx.scene.control.SelectionMode.MULTIPLE);
		/*
		 ...
		 */
		expressionsTreeView.rootProperty().bind(expressionsModel.rootItemProperty());
	...
	if (context == null || context.strip().isEmpty()) {
		messageLabel.setText("saisie vide");
		return;
	}
		try {
		boolean added = expressionsModel.parse(context);
		messageLabel.setText(added ? "l'expression a été ajoutée" : "l'expression n'a pas été ajoutée");
	} catch (ParserException e) {
		messageLabel.setText("Vérifiez que les constantes sont  des " + specimen.get().getClass().getSimpleName());
	...
	public void onEditCommitAction(CellEditEvent<ExpressionDisplay<? extends Number>, String> event)
	{
		 ...
		 * and return
		 */
		if (Objects.equals(oldContext, newContext)) {
			messageLabel.setText("aucun changement");
			return;
		}
		/*
		 ...
		 */
		try {
			expressionsModel.reparse(expression, newContext);
			messageLabel.setText("l'expression a été changée");
		} catch (ParserException e) {
			messageLabel.setText("Parser error occured");
			Alert alert = new Alert(Alert.AlertType.ERROR);
			alert.setTitle(title);
			alert.setHeaderText("Parser error");
			alert.setContentText(e.getMessage());
			...
			logger.severe(title + " unexpected error: " + e.getMessage());
		}
	}
	...
	public void onDeleteSelectedAction(ActionEvent event)
	{
		 ...
		 */
		List<ExpressionDisplay<Number>> selectedDisplays =
	    expressionsTableView.getSelectionModel().getSelectedItems();

	if (selectedDisplays == null || selectedDisplays.isEmpty()) {
		messageLabel.setText("aucune expr selectionnée");
		return;
	}

	List<Expression<Number>> toRemove = new ArrayList<>();
	for (ExpressionDisplay<Number> display : selectedDisplays) {
	toRemove.add(display.getExpression());
}
		/*
		...
		for (Expression<Number> expr : toRemove) {
			if (expressionsModel.remove(expr)) {
				removed = true;
			}
		}
		/*
	...
	public void onSelectAllAction(ActionEvent event)
	{
		...
		messageLabel.setText("toutes les expr sélectionnées");
	}
	...
	public void onDeselectAllAction(ActionEvent event)
	{
		 ...
		 */
		expressionsTableView.getSelectionModel().clearSelection();
		messageLabel.setText("toutes les expr désélectionnées");
		}
	...
	public void onNewAction(ActionEvent event)
	{
		...
		messageLabel.setText("nouvelle action effectuée");
	}
	...
	public void onRevertAction(ActionEvent event)
	{
		...
		onClearFilterAction(null);
		expressionsModel.clear();
		if(expressionsModel.getFile()!=null){
			loadFile(expressionsModel.getFile(), false);
		}
		else{
			Alert al = new Alert(Alert.AlertType.WARNING);
			al.setTitle("pas de fichier");
			al.showAndWait();
		}
	}
	 ...
	 * @param event Even t associated with this action
	 */
	public void onSelectNumberType(ActionEvent event)
	{
		 ...
		 */
		if (newSpecimen != null && !newSpecimen.getClass().equals(oldSpecimen.getClass())) {
			expressionsModel.clear();
	...
	public void onClearFilterAction(ActionEvent event)
	{
		 ...
		 */
		searchField.clear();
                if (!terminalTypeCombobox.getItems().isEmpty()) {
                        terminalTypeCombobox.getSelectionModel().select(0);
                }
                if (!binaryTypeCombobox.getItems().isEmpty()) {
                        binaryTypeCombobox.getSelectionModel().select(0);
                }
	...
	public void onDisplayButtonsWithStyle(ActionEvent event)
	{
		...
		{
			Object source = event.getSource();
		if (source == graphicsOnlyRadiomenuItem) {
			contentDisplay = ContentDisplay.GRAPHIC_ONLY;
		} else if (source == textAndGraphicsRadiomenuItem) {
			contentDisplay = ContentDisplay.LEFT;
		} else if (source == textOnlyRadiomenuItem) {
		 ...
		 */
		for (Labeled elem : this.styleableButtons) {
			elem.setContentDisplay(contentDisplay);
		}
		/*
		 * TODO Set message in #messageLabel indicating content display
		 */
		String msg = switch (contentDisplay) {
			case GRAPHIC_ONLY -> "graphique uniquement";
			case TEXT_ONLY -> "texte uniquement";
			case LEFT -> "texte + graphique";
			default -> "inconnu";
		};
		this.messageLabel.setText("le style des boutons  : " + msg);	}
	...
	public void onSetLoggerLevelAction(ActionEvent event)
	{
		 ...
		 */
		if (source == infoWarningSevereRadiomenuItem) {
			loggerLevel = Level.INFO;
	} else if (source == warningSevereRadiomenuItem) {
			loggerLevel = Level.WARNING;
	} else if (source == severeRadiomenuItem) {
			loggerLevel = Level.SEVERE;
	} else if (source == offRadiomenuItem) {
	...
	public void onAboutAction(ActionEvent event)
	{
		...
		String header = "Expressions Manager";
                ...
                alert.setTitle("About " + header);
                alert.setHeaderText(header + " v2.0");
                alert.setContentText("A simple application to manage math expressions");
